...
from queens.randomfields.random_field_gen_fourier import RandomFieldGenFourier


class RandomFieldGenFourier1D(RandomFieldGenFourier):
    """Fourier series based 1D random field generator.
    ...
    def __init__(
        ...
        for k1 in range(0, self.m):
            sum_ck = sum_ck + (
                self.compute_expansion_coefficient(k1, self.largest_length, self.corr_length)
            )
            num_ck = num_ck + 1
            self.kb[index, 0] = k1
            index = index + 1
