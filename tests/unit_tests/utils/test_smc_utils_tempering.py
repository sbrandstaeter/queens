...
from queens.utils import sequential_monte_carlo
...
@pytest.fixture(
    ...
    scope="module",
    params=[
        ("bayes", type(sequential_monte_carlo.temper_logpdf_bayes)),
        ("generic", type(sequential_monte_carlo.temper_logpdf_generic)),
    ],
)
...
def test_temper_factory(temper_keyword_and_temper_type):
    ...
    temper_type = sequential_monte_carlo.temper_factory(temper)
...
def test_temper_factory_invalid():
    ...
    with pytest.raises(ValueError, match=r"Unknown type.*"):
        sequential_monte_carlo.temper_factory(temper)
...
def test_temper_logpdf_bayes(logpdf0, logpdf1, temper_parameter):
    ...
    tempered_logpdf = sequential_monte_carlo.temper_logpdf_bayes(logpdf0, logpdf1, temper_parameter)
...
def test_temper_logpdf_bayes_posinf_invalid(logpdf0, temper_parameter):
    ...
    with pytest.raises(ValueError):
        sequential_monte_carlo.temper_logpdf_bayes(np.inf, logpdf0, temper_parameter)
    with pytest.raises(ValueError):
        sequential_monte_carlo.temper_logpdf_bayes(logpdf0, np.inf, temper_parameter)
...
def test_temper_logpdf_generic(logpdf0, logpdf1, temper_parameter):
    ...
    tempered_logpdf = sequential_monte_carlo.temper_logpdf_generic(
        logpdf0, logpdf1, temper_parameter
    )
    assert np.isclose(tempered_logpdf, tempered_logpdf_sol)
...
def test_temper_logpdf_generic_posinf_invalid(logpdf0, temper_parameter):
    ...
    with pytest.raises(ValueError):
        sequential_monte_carlo.temper_logpdf_generic(np.inf, logpdf0, temper_parameter)
    with pytest.raises(ValueError):
        sequential_monte_carlo.temper_logpdf_generic(logpdf0, np.inf, temper_parameter)
