...
class SobolGratietDesignerLHD(AbstractDesigner):
    ...
    def __init__(self, params, seed, num_samples):
        """ Initialize Pseudo Saltelli designer

        Args:
            params (dict):      Dictionnary with the definition of the problem.
                                Usually contains the name of each parameter,
                                and for each parameter its type, size, minimum and
                                maximum values, its distribution and its distribution
                                parameters.
