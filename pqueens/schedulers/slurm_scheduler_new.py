...
class SlurmScheduler(Scheduler):
    ...
    def __init__(self, scheduler_name, num_procs_per_node, num_nodes, walltime, user_mail, output):
        """
        Args:
        ...
        self.job_id = None # this will be added by , parsed as argument form connect_to... method that has the argument from resource.py call which needs to be changed
    ...
    def from_config_create_scheduler(cls, config, scheduler_name):
        ...
        return cls(scheduler_name, num_procs_per_node, num_nodes, walltime, user_mail, output, base_settings)
...
def alive(self, process_id): #formulated without ssh
        ...
        try:
            # join lists
            command_list = ['squeue --job', str(process_id)]
    ...
    def submit(self, job_id, experiment_name, batch, experiment_dir,
               database_address, driver_options): #TODO command which job submitted job_id and batch are added over resources.py call
        ...
        cmdlist_remote_main = [singularity, ./remote_main.py, remote_args]
        cmd_remote_main = ' '.join(cmdlist_remote_main)

        #### Call parents "connect_computing_machine" method
        super connect_to_resource(cmd_remote_main)

        ###############################################
        ################################################
        # below old suff that needs to be checked but can be deleted probably
        output, std_err = process.communicate()
        process.stdin.close()

        # get the process id from text output
        match = self.get_process_id_from_output(output) #TODO: Check how to do this
        try:
            return int(match)
        except:
