...
class TestPDFEstimation(unittest.TestCase):
    ...
    def test_compute_kernel_bandwidth(self):
        ...
        np.testing.assert_almost_equal(bandwidth, 0.43846153846153846, 7)
    ...
    def test_density_estimation(self):
        ...
        pdf_desired = np.array([0.2374548, 0.3022495, 0.3665407, 0.4094495, 0.4211095,
                                0.4052543, 0.3688492, 0.3189032, 0.2642803, 0.2129013])

        np.testing.assert_almost_equal(pdf_estimate, pdf_desired, 7)

        # test without support points
        pdf_estimate, supp_points = estimate_pdf(self.samples, self.bandwidth,
                                                 support_points=None)
