...
class TestRandomFieldGeneratorFourier2D(unittest.TestCase):
    ...
    def test_values_at_location(self):
        ...
        np.testing.assert_allclose(my_vals,np.array([-0.39252351, -0.59235608,
                                                     0.0266424, 0.23892774,
                                                     0.61143994, 0.92525213,
                                                     -0.80441616]),1e-07,1e-07)
    ...
    def test_marginal_distribution(self):
        ...
        for i in range(200):
            xi = np.random.randn(self.my_stoch_dim,1)
            my_vals[0, i] = self.my_field_generator.evaluate_field_at_location(np.array([[25,25]]), xi)
