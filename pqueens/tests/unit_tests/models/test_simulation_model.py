...
from pqueens.utils.gradient_handler import CallableGrad
...
def test_evaluate_and_gradient():
    ...
    model_obj.grad_handler = CallableGrad("my_grad_handler")
    response, gradient_response = model_obj.evaluate_and_gradient(samples)
    assert response == 1.0
    ...
    response, gradient_response = model_obj.evaluate_and_gradient(
        samples, grad_objective_fun=gradient_objective
    )
    assert response == 1.0
