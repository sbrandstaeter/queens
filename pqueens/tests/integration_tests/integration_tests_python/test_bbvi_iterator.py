...
def test_bbvi_iterator_park91a_hifi(inputdir, tmpdir, design_and_write_experimental_data_to_csv):
    ...
    # Actual tests
    assert np.abs(results['variational_distr']['mu'][0] - 0.5) < 0.015
    ...
    # assert results['variational_distr']['noise_std'] < 0.05 this was removed in the iterator
...
def test_bbvi_density_match(
    ...
    target_pdf = target_density('dummy', x=opt_variational_samples, pdf=True).flatten()
    kl_divergence = entropy(variational_pdf, target_pdf)
    assert kl_divergence < 5.0


@pytest.fixture()
def dummy_bbvi_instance(tmpdir):
    #  ----- interesting params one might want to change ---------------------------
    n_samples_per_iter = 10
    min_requ_relative_change_variational_params = 0.002
    num_variables = 5
    learning_rate = 0.2
