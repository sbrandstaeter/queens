...
class TestRandomFieldGeneratorFourier3D(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.total_terms             = 40
        self.loc                     = np.array([[0, 0, 0],[0, 0, 10],
                                                 [0, 10, 0],[10, 0, 0],
                                                 [0, 0, 25], [0, 25, 0],
                                                 [25, 0, 0],[0, 0, 100],
                                                 [0, 100, 0], [100, 0, 0]])
    ...
    def test_values_at_location(self):
        ...
        ref_vals = np.array([-0.272606175022982, -0.364627863004368,
                             0.126620737359761, 0.460434864806447,
                             0.293453869499679, -0.156924505159762,
                             0.310580566207436, -0.431044039109896,
                             0.265531834295388, -0.853659094570138])
    ...
    def test_marginal_distribution(self):
        ...
        for i in range(200):
            xi = np.random.randn(self.my_stoch_dim,1)
            my_vals[0, i] = self.my_field_generator.evaluate_field_at_location(np.array([[25,25,25]]), xi)
