...
def convert_array_to_db_dict(numpy_array):
    """
    Convert numpy arrays in a dictionary format that can be understood by MongoDb.
...
def convert_nested_data_to_db_dict(u_container):
    """ 
    Restructure nested input data formats into dictionaries that are compatible
    with the MongoDB
...
def convert_nested_db_dicts_to_lists_or_arrays(db_data):
    ...
    to return the original input format of either list or dict type
