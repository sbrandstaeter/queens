...
class SobolAnalyzer(object):
    ...
    def compute_first_order_sensitivity_indice(self,Y,Y_tilde):
        ...
        H = self.generate_bootstrap_samples(len(Y))
        m = len(Y)
    ...
    def compute_second_order_sensitivity_indice(self,Y, Y_tilde, Y_tilde_bis, Y_tilde_ter):
        ...
        for k in range(self.output_samples):
            num = (np.sum(Y*Y_tilde)/m-(np.sum(Y+Y_tilde)/(2*m))**2)
            den = (sum(Y**2)/m-(np.sum(Y+Y_tilde)/(2*m))**2)
            S_1 = self.compute_first_order_sensitivity_indice(Y, Y_tilde_bis)
            S_2 = self.compute_first_order_sensitivity_indice(Y, Y_tilde_ter)
            ...
            for l in range(1,self.num_bootstrap_samples):
                ...
                num_b = (np.sum(Y_b*Y_tilde_b)/m-(np.sum(Y_b+Y_tilde_b)/(2*m))**2)
                den_b = (sum(Y_b**2)/m-(np.sum(Y_b+Y_tilde_b)/(2*m))**2)
                S_1_b = self.compute_first_order_sensitivity_indice(Y_b, Y_tilde_bis_b)
                S_2_b = self.compute_first_order_sensitivity_indice(Y_b, Y_tilde_ter_b)
    ...
    def analyze(self,Y):
        ...
        for j in range(dim):
            ...
            S['S1_conf'][j] = 0
            S['ST'][j] = self.compute_total_order_sensitivity_indice(Y[:,0], Y[:,nb_combi-j-2])
            S['ST_conf'][j] = 0
        ...
        if self.calc_second_order:
            for j in range(dim):
                for k in range(j + 1, dim):
                    S['S2'][j, k] = self.compute_second_order_sensitivity_indice(Y[:,0],
