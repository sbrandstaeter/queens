"""modified version of sklearn's decision_boundary.py to plot higher-dimensional decision boundaries for neural iterator"""
    ...
    def from_estimator(
        ...
        **kwargs : dict
        ...
        if hasattr(X, "iloc"):
            ...
            n_start = params[0]
            n_between = params[1] - params[0] - 1
            n_end = n_params - params[1] - 1
            X_grid = np.c_[np.zeros(shape=(grid_resolution ** 2, n_start)), xx0.ravel(), np.zeros(shape=(grid_resolution ** 2, n_between)), xx1.ravel(), np.zeros(shape=(grid_resolution ** 2, n_end))]
